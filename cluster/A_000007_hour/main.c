...
int main(int argc, char *argv[])
{
	...
	if(argc < 2)
	{
        ...
        fprintf(stdout, LCS_RETURN_STATUS, 
            "A_000009_hour", "EXIT_FAILURE", err);
		return EXIT_FAILURE;
	}
    ...
    fprintf(stdout, LCS_RETURN_STATUS, "A_000007_hour", "EXIT_SUCCESS", err);
...
void lcs_unit_gen(char *dir, char *sub, char *ana, int *err)
{
	...
	sprintf(cmd, "%s %s > /dev/null", subprocess, path, sub, ana);	
...
void lcs_unit_config(char *dir, char *sub, char *ana, char *path, int *err)
{
	...
	fprintf(fp, "prefixshmkey_0003:%s", lcs_info_cfg.prefixshmkey);
	fclose(fp);
}
/***********************************************************************************/
