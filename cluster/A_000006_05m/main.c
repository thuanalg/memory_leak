...
static int  lcs_exec_cbtrue(void* fp, int n, char** val, char** headers);
/***********************************************************************************/
...
//	char cmd[2048];
	...
	{
		*err = __LINE__;
		fprintf(stdout, "No dbpath.\n");
		return;	
	}

	memset(query, 0, 1024);
	sprintf(query, "../sql/%s.sql", name);
	lcs_comm_filexist(query, &isexist);
	if(!isexist)
	{
		*err = __LINE__;
...
//	memset(cmd, 0, 2048);
//	sprintf(cmd, 
//		"sqlite3 %s/lcs.db < ../sql/%s.sql > %s/%s.analysis", 
//			path, name, path, name);
//	fprintf(stdout, "cmd: \n\%s\n", cmd);
...
int  lcs_exec_cbtrue(void* fp, int n, char** val, char** headers)
{
	...
	do
	{
		fprintf((FILE *)fp, "%s\t", (val[i]) ? (val[i]):("NULL") );
		++i;
	}
	while(i < n);	
	fprintf((FILE *)fp, "\n");
