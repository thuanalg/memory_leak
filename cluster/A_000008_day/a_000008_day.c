...
typedef struct LCS_ANA008_HOUR
{
	char sub[6];
	char ana[64];
	char dir[1024];
...
int main(int argc, char *argv[])
{
	...
	{
		err = __LINE__;
		fprintf(stdout, LCS_RETURN_STATUS, "A_000008_day", "EXIT_FAILURE", err);
		return EXIT_FAILURE;
	}
	gettimeofday(&t0, 0);
	lcs_cfg_file = argv[1];
	...
	fprintf(stdout, LCS_RETURN_STATUS, "A_000008_day", "EXIT_SUCCESS", err);
	return EXIT_SUCCESS;
}
/***********************************************************************************/
...
(char *day, char *hour, char *ana, char *path, int *err)
{
	...
	fprintf(fp, "subprocess_0003:../A_000008_05m/lcs_A_000008_05m\n");
	fprintf(fp, "prefixshmkey_0004:%s", lcs_info_cfg.prefixshmkey);
	fclose(fp);
}
/***********************************************************************************/
void *lcs_ana_008(void* arg)
{
	int err 				= 0;
	LCS_ANA008_HOUR *p 		= 0;
