...
typedef struct LCS_ANA009_HOUR
{
	char sub[6];
	char ana[64];
	char dir[1024];
...
int main(int argc, char *argv[])
{
	...
	{
		err = __LINE__;
		fprintf(stdout, LCS_RETURN_STATUS, "A_000005_day", "EXIT_FAILURE", err);
		return EXIT_FAILURE;
	}
	gettimeofday(&t0, 0);
	lcs_cfg_file = argv[1];
	...
	fprintf(stdout, LCS_RETURN_STATUS, "A_000005_day", "EXIT_SUCCESS", err);
	return EXIT_SUCCESS;
}
/***********************************************************************************/
...
void lcs_hcollect_ana(char *dir, char **sub, int *err)
{
	...
	lcs_comm_filexist(dest, &having);
	if(having)
	{
		char *text = 0;
		lcs_file_text(dest, &text, err);
		if(text)		
		{
			if(text[0])
			{
				lcs_hcollect005_sum(dest, text, err);				
			}
			free(text);
		}
	}
...
void lcs_dgen_subtask(char *day, char *hour, char *ana, int *err)
{
	...
	sprintf(cmd, "%s %s > /dev/null", 
		lcs_info_cfg.subprocess,path);
...
(char *day, char *hour, char *ana, char *path, int *err)
{
	...
	fprintf(fp, "subprocess_0003:../A_000005_05m/lcs_A_000005_05m");
	fclose(fp);
}
/***********************************************************************************/
void *lcs_ana_005(void* arg)
{
	int err 				= 0;
	LCS_ANA009_HOUR *p 		= 0;
