...
int main(int argc, char *argv[])
{
	...
	{
		err = __LINE__;
		fprintf(stdout, LCS_RETURN_STATUS, "A_000009_day", "EXIT_FAILURE", err);
		return EXIT_FAILURE;
	}
	gettimeofday(&t0, 0);
	lcs_cfg_file = argv[1];
	...
	fprintf(stdout, LCS_RETURN_STATUS, "A_000009_day", "EXIT_SUCCESS", err);
	return EXIT_SUCCESS;
}
/***********************************************************************************/
...
void lcs_hcollect_ana(char *dir, char **sub, int *err)
{
	...
	while(sub[i])	
	{
		...
		if(!isexist)
		{
			...
			pthread_create(&id, 0, lcs_ana_009, p);
			//lcs_ana_009(p);
			count++;
		}
		++i;
...
(char *day, char *hour, char *ana, char *path, int *err)
{
	...
	fprintf(fp, "subprocess_0003:../A_000009_05m/lcs_A_000009_05m\n");
	fprintf(fp, "prefixshmkey_0004:%s", lcs_info_cfg.prefixshmkey);
	fclose(fp);
}
/***********************************************************************************/
void *lcs_ana_009(void* arg)
{
	int err 				= 0;
	LCS_ANA009_HOUR *p 		= 0;
