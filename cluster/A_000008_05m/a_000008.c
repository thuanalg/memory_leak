...
int  lcs_exec_cbtrue(void* data, int ncols, char** val, char** headers)
{
	...
	/* +++++++++++++++++++++*/	
	//txt = (p0? ( (strlen(p0->application_id) == 16) ? "VPBFC": "VPBANK")  : "")   ;
	++i;
	//FINANCIER_ID
	fprintf((FILE*)data, "%s\t", val[0][0] ?  
		(p0? ( (strlen(p0->application_id) == 16) ? "VPBFC": "VPBANK")  : "")    : ( "" ));
	++i;
	/* +++++++++++++++++++++*/	
	//APPL_ID
	fprintf((FILE*)data, "%s\t", p0? (p0->application_id) : (""));
	++i;
	/* +++++++++++++++++++++*/	
	//CUST_ID
	fprintf((FILE*)data, "%s\t", (val[i][0])? (p0? (p0->customer_id) : ("")) : "");
	++i;
	/* +++++++++++++++++++++*/	
	//USER_ID
	txt = (p0 ? ( (p0->skill == 1) ? "PDS00000" : "ES100000"  ) : "") ;
	fprintf((FILE*)data, "%s\t", (val[i][0])? ( txt ) : "");
	++i;
	/* +++++++++++++++++++++*/	
	//ACTION_DATE
	...
	if(val[i][1] == 0)
	{
		fprintf((FILE*) data, "%s\t", (p0->skill==1) ? "PD_NKP" : "NKP");
	}
	else if (val[i][2] == 0)
	{
		fprintf((FILE*) data, "%s\t", "ANSWER");
	}
	else if (val[i][2])
	{
		fprintf((FILE*) data, "%s\t", (p0->skill==1) ? "PD_NAB" : "NAB");
	}
	else
	{
		fprintf((FILE*) data, "\t");
	...
	do
	{
		fprintf((FILE*) data, "%s\t", val[i + count]);
		++count;
	}
	while( count < 5 );
	i += count;
	...
	if(val[i][0])
	{
		fprintf((FILE*) data, "%s\t", (p0->skill==1)  ? "PDS" : "ECS" );
	}
	else
	{
		fprintf((FILE*) data, "\t");
	...
	if(val[i][0])
	{
		fprintf((FILE*) data, "%s_%s_", "Uptrail_Auto", (p0? p0->phone_number : ""));
		if(strcmp(val[i], "NO ANSWER") ==  0)
		{
			fprintf((FILE*) data, "%s\t", ( p0? ( (p0->skill == 1) ? "PD_NKP" : "NKP") : "" ));
		}
		else
		{
			fprintf((FILE*) data, "%s\t", ( p0? ( (p0->skill == 1) ? "PD_NAP" : "NAB") : "" ));
		}
		fprintf((FILE*) data, "\t");
	}
	else
