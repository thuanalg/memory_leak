...
#define IGX_SKYLAB_PATH  													"/77b5ea39-1dc2-4875-8b4c-8700972db7ff_"
#define IGX_SKYLAB_SHARED_MODE      							(S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
...
{
  int err = 0;
	char *path = (char *)IGX_SKYLAB_PATH;
  do
  {
    if(skylab_data_shm)
    {
      err = munmap(skylab_data_shm, LIST_SHARED_TRAFFIC_SZ);
    }
...
void *skylab_open_shm()
{
  ...
  do
  {
    err = _X(init_service)(path, &skylab_data_shm, LIST_SHARED_TRAFFIC_SZ);
  }
  while(0);
	if(!err){
		return skylab_data_shm;	
	}
...
int _X(init_service)(char *path, void **out, int sz)
{
  ...
  int shm = 0;
  void *data = 0;
  //sprintf(key, "%s%s", IGX_SKYLAB_PATH, key);
  ...
  do {
    ...
    shm = shm_open(key, O_CREAT | O_RDWR | O_EXCL, IGX_SKYLAB_SHARED_MODE);  
    if(shm >= 0)
    {
			// It is creating mode.
      create = 1;
    ...
    else
    {
		// It is reading/writing mode.
      	shm = shm_open(key, O_RDWR | O_EXCL, IGX_SKYLAB_SHARED_MODE);  
    }
...
int skylab_read_shm(LIST_SHARED_TRAFFIC *p, char **data, char clean)
{
	int rs = 0;
	char *tmp = 0;
	int errx = 0;
...
void analyse_data()
{
	...
	LIST_SHARED_TRAFFIC *p = (LIST_SHARED_TRAFFIC *)skylab_data_shm;
	...
	n = skylab_read_shm(p, (char **)&data, 0);
...
int skylab_take_traffic_data(char **data)
{
	int n = 0;
	char *p = 0;
